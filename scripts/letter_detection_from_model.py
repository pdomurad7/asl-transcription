...
# data = pd.read_csv("data_without_j_z_400_3_rfc.csv")
...
from nn_model_training.letters import Letters
...
model = load_model('nn_model_training/models/model_rotated.h5')
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        cv.putText(img, str(Letters(np.argmax(prediction[0]))), (50, 50), cv.FONT_HERSHEY_SIMPLEX, 1, (128, 0, 255), 2, cv.LINE_AA)
        cv.putText(img, str(prediction[0][np.argmax(prediction[0])]), (50, 100), cv.FONT_HERSHEY_SIMPLEX, 1, (128, 0, 255), 2, cv.LINE_AA)
